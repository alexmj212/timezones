class Clock {
    ...
    setSeconds() {
        ...
        // set x
        secondHandElement.style.top = this.secondHand.x = this.clock.center.x + (this.clock.radius * Math.sin(this.secondHand.position)) + 'px';

        // set y
        secondHandElement.style.left = this.secondHand.y = this.clock.center.y + (this.clock.radius * Math.cos(this.secondHand.position)) + 'px';
    ...
    setTicks() {
        ...
        // set x
        tickElement.style.top = this.clock.center.x + (this.clock.radius * Math.sin(((-1) * Math.PI) / 2)) + tickElement.getBoundingClientRect().width + 2 + 'px';

        // set y
        tickElement.style.left = this.clock.center.y + (this.clock.radius * Math.cos(Math.PI / 2)) + tickElement.getBoundingClientRect().width + 1 + 'px';

    }

    initialize() {
        var date = new Date();
        this.clock.radius = this.element.querySelector('.clock').clientWidth / 2;
        var offset = this.element.querySelector('.second-hand').clientWidth / 2 + 1;
        this.clock['center'] = { x: (this.clock.radius - offset), y: (this.clock.radius - offset) };
